...
module TitleExpert
  # Information about the status of an LRBG request.
  class LrbgResponseCommon
    # If the status is Complete, the success message (if any) received from the LRBG. May be null.
    attr_accessor :success_message

    # If the status is Failed, the failure code (if any) received from the LRBG. May be null.
    attr_accessor :failure_code

    # If the status is Failed, the failure message (if any) received from the LRBG. May be null.
    attr_accessor :failure_message

    # The actual price charged by the LRBG for this request, if known. May be null if not known.
    attr_accessor :actual_price
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'FailureCode')
        self.failure_code = attributes[:'FailureCode']
      end
      ...
      if attributes.has_key?(:'ActualPrice')
        self.actual_price = attributes[:'ActualPrice']
      end
    ...
    def hash
      [success_message, failure_code, failure_message, actual_price, id, response_uri, reference, deadline, status, status_last_changed, acknowledge_uri, acknowledged].hash
    end
